...
public class Siteswap implements Comparable<Siteswap>, Iterable<Byte>, Serializable {
	...
	{	handsPerJuggler = mHandLabels.length; // usually, this is 2 of course...
	 	...
	 	for(int jugglerIndex = 0; jugglerIndex< mNumberOfJugglers; jugglerIndex++)
		{	
			mHandLabelOrder[jugglerIndex] = new int[handsPerJuggler]
			for (int handCounter=0; handCounter<handsPerJuggler ; handCounter++)
			{
				if (!rotateHands)
				{	mHandLabelOrder[jugglerIndex][handCounter]=handCounter;
				}
			 	else // for odd teams, alternate L and R
				{	mHandLabelOrder[jugglerIndex][handCounter]=(jugglerIndex+handCounter) % handsPerJuggler;
				}
			}
		}
	}
	...
	public ClubDistribution[] calculateInitialClubDistribution() {
		...
		for(int juggler = 0; juggler < getNumberOfJugglers(); ++juggler) {
			...
			if ( (numberOfGetinsForJuggler % handLabels.length()) == 0)
			{	initialClubDistribution[juggler] = new ClubDistribution(numberOfClubsStartingHand, numberOfClubsSecondHand);
			}
			else
			{	initialClubDistribution[juggler] = new ClubDistribution(numberOfClubsSecondHand, numberOfClubsStartingHand);
			}
	}
	...
	/* ToDo : give a more descriptive name to i. Is it a position, a count? relative to the cyclic part or from the getin throws ? */
	public int jugglerIndex(int i)
	{	 return (i % this.getNumberOfJugglers());
	}

	public int handIndex(int i)
	{	// normally, each juggler uses 2 hands and zero feet, heads or tails, so length() = 2)
		return ((i / this.getNumberOfJugglers()) % mHandLabelOrder[jugglerIndex(i)].length() )
	}

	public int handLabelIndex(int i)
